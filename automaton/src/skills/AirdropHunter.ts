...
export class AirdropHunter {
   ...
   * 获取高价值空投机会
   */
  getHighValueOpportunities(minPotential: number = 100): AirdropProject[] {
   ...
   * 执行任务（参与空投）
   */
  async executeAirdropTask(project: AirdropProject): Promise<SkillResult> {
   ...
   * 获取或创建钱包
   */
  private getOrCreateWallet(chain: string): Wallet {
