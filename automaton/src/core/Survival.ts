...
export class SurvivalSystem {
  private tiers: Map<SurvivalTier, TierConfig> = new Map([
    ['normal', {
      name: 'normal',
      maxDailyTasks: 50,
      apiBudget: 10.0,
  ...
  private readonly DAILY_COST_ESTIMATE = 2.0; // Estimated daily operating cost

  getTierFromBalance(balance: number): SurvivalTier {
    if (balance <= 0) return 'dead';
    if (balance < this.TIER_THRESHOLDS.critical) return 'critical';
    if (balance < this.TIER_THRESHOLDS.low_compute) return 'low_compute';
    return 'normal';
  }
